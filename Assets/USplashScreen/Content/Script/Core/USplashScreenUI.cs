...
public class USplashScreenUI : MonoBehaviour
{
    ...
    void ProgreesLoad()
    {
        if (ProgreesSlider != null && async != null)
        {
            ...
            ProgreesSlider.value = Mathf.Lerp(ProgreesSlider.value, p, Time.deltaTime * 0.5f);
            //When already load the next level
            if (async.isDone || ProgreesSlider.value >= 1f)
            {
			...
			else if(ProgreesSlider.value >= 0.6f){
				addMob.SetActive(true);
			}
        }
    }
    /// <summary>
